...
const Contact = () => {
  ...
  const updateToastPosition = () => {
    if (window.innerWidth <= 600) {
      setToastPosition("bottom-right"); // For mobile
    } else {
      setToastPosition("top-right"); // For desktop
    }
  };
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm("service_un4eowo", "template_blvk95r", formRef.current, {
        publicKey: "dCTp2b9KyxnXALjbT",
      })
      .then(
        () => {
    ...
    >
      <motion.div className="textContainer" variants={variants}>
        ...
        <motion.div className="item" variants={variants}>
          <h2>Mail</h2>
          <span>patelparam9304@gmail.com</span>
        </motion.div>
      </motion.div>
      <div className="formContainer">
        ...
        >
          <svg version="1.1" viewBox="0 0 32.666 32.666">
            {" "}
            <motion.path
              strokeWidth={0.2}
              fill="none"
